  ...
  <div class="auth-shell">
    <div class="auth-card">
      ...
      <form class="auth-form" @submit.prevent="handleRegister">
        ...
        <div class="field">
          <label>密码</label>
          <input v-model="form.password" type="password" placeholder="至少 6 位密码" autocomplete="new-password" />
        </div>
...
async function handleRegister() {
  if (!form.value.username || form.value.password.length < 6) return
  loading.value = true
...
.auth-shell { min-height: 100vh; display: flex; align-items: center; justify-content: center;
  background: radial-gradient(ellipse at 20% 50%, rgba(59,130,246,0.06) 0%, transparent 50%),
              ...
              var(--color-bg); padding: var(--space-6); }
.auth-card { width: 400px; background: var(--color-surface); border-radius: var(--radius-xl);
  box-shadow: var(--shadow-xl); padding: var(--space-10) var(--space-8); }
.auth-brand { text-align: center; margin-bottom: var(--space-8); }
.brand-mark { width: 52px; height: 52px; margin: 0 auto var(--space-4);
  background: linear-gradient(135deg, #3b82f6, #2563eb); border-radius: 14px;
  display: flex; align-items: center; justify-content: center; color: #fff; }
.auth-brand h1 { font-size: 22px; font-weight: 700; letter-spacing: -0.02em; }
.auth-brand p { font-size: var(--text-sm); color: var(--color-text-secondary); margin-top: 4px; }
.field { margin-bottom: var(--space-4); }
.field label { display: block; font-size: var(--text-sm); font-weight: 500; margin-bottom: 6px; }
.field input { width: 100%; height: 44px; padding: 0 var(--space-3); background: var(--color-bg);
  border: 1.5px solid var(--color-border); border-radius: var(--radius-sm); font-size: var(--text-base);
...
.btn-primary { width: 100%; height: 44px; background: linear-gradient(135deg, #3b82f6, #2563eb); color: #fff;
  border: none; border-radius: var(--radius-sm); font-size: 15px; font-weight: 600; cursor: pointer;
  margin-top: var(--space-2); transition: all var(--transition-fast); display: flex; align-items: center; justify-content: center; }
...
.auth-footer { text-align: center; margin-top: var(--space-6); font-size: var(--text-sm); color: var(--color-text-secondary); }
